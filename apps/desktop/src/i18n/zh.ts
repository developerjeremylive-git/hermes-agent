...
export const zh = defineLocale({
  ...
  settings: {
    ...
    gitHub: {
      ...
      waiting: '等待 GitHub 认证…',
      loginStarting: '正在启动 GitHub 登录…',
      loginUrlHint: '在浏览器中打开此 URL 并输入代码：',
      enterCode: '在浏览器中输入此代码以链接您的 GitHub 帐户。',
      openBrowser: '在浏览器中打开 GitHub',
      failed: 'GitHub 认证失败或已取消。',
      connected: '已连接',
      logout: '退出登录',
      workingFolder: '工作文件夹',
      workingFolderHint: 'Hermes 会话在此文件夹中运行，当您开始新聊天时。',
      noWorkingFolder: '未设置工作文件夹',
      chooseWorkingFolder: '选择文件夹',
      createRepo: '创建仓库',
      createRepoHint: '在没有 git 仓库的文件夹中初始化 git 仓库，然后将其用作工作文件夹。',
      creatingRepo: '正在创建仓库…',
      repoCreated: '仓库已创建',
      ...
      conflicted: '有冲突',
      conflictContentUnavailable: '内容不可用（二进制或太大无法预览）',
      acceptOurs: '接受我们的',
      acceptTheirs: '接受他们的',
      acceptBoth: '接受两者',
      continueMerge: '继续合并',
      continueMergeHint: '通过创建合并提交来完成合并',
      confirmAbort: '确认中止',
      abortMerge: '中止合并',
      mergeCompleted: '合并完成',
      mergeAborted: '合并已中止',
      ...
      resolveConflictsWithAgentPrompt:
        '解决阻止此仓库上次同步的任何问题。首先用 git status 检查它。如果任何文件中存在冲突标记（<<<<<<<, =======,>>>>>>>）：打开每个冲突文件，决定正确的解决方案，应用编辑，并用 git add 暂存已解决的文件。如果 git status 显示合并仍在进行中，用 git commit --no-edit 完成它。如果 git stash list 显示名为 hermes-sync-autostash 的条目：当没有合并进行时，冲突标记已包含您恢复的本地更改，因此用 git stash drop 删除冗余条目；当合并进行时，用 git stash apply 恢复您的本地更改，解决并暂存它创建的任何冲突，然后用 git stash drop 删除该条目。保持当前分支提交完整。报告您更改的内容和原因。',
      notInsideRepo: '该文件夹不在 git 仓库内',
      workingFolderUpdated: '工作文件夹已更新',
      configGlobal: '全局配置',
      configLocal: '本地配置',
      configSetSuccess: 'GitHub 帐户已配置',
      configSetFailed: '配置 GitHub 帐户失败',
      setAccountTitle: '设置 GitHub 帐户',
      setAccountHint: '输入用于推送的 GitHub 帐户。',
      usernameLabel: 'GitHub 用户名',
      openRepoOnGitLab: '在 GitLab 上打开',
      remoteRepositories: '远程仓库',
      remoteRepositoriesHint: '浏览和克隆来自您的 GitHub 帐户的仓库。',
      loadRepos: '加载仓库',
      loadingRepos: '正在加载仓库…',
    ...
    gitLab: {
      ...
      connectHint: '使用个人访问令牌连接以在 Hermes 中使用 GitLab。',
      connected: '已连接',
      logout: '退出登录',
      loginFailed: '无法使用该令牌登录 GitLab。',
      tokenTitle: '使用个人访问令牌连接',
      tokenHint: '在 GitLab 上创建具有 `api` 和 `read_user` 作用域的令牌，然后将其粘贴到这里。',
      tokenLabel: '个人访问令牌',
      ...
      localRepositoriesHint: '在此计算机上找到的托管在 GitLab 上的仓库。选择一个用作工作文件夹。',
      jDriveRepositories: 'AI Products 仓库',
      jDriveRepositoriesHint: '在 J:\\AI_Products 中找到的托管在 GitLab 上的仓库。选择一个用作工作文件夹。',
      sortByName: '名称',
      sortByLastCommit: '最后提交',
      openRepoFolder: '打开文件夹',
      openRepoOnGitHub: '在 GitHub 上打开',
      ...
      configGlobal: '全局配置',
      configLocal: '本地配置',
      configSetSuccess: 'GitLab 帐户已配置',
      configSetFailed: '配置 GitLab 帐户失败',
      setAccountTitle: '设置 GitLab 帐户',
      setAccountHint: '输入用于推送的 GitLab 帐户。',
      usernameLabel: 'GitLab 用户名',
      remoteRepositories: '远程仓库',
      remoteRepositoriesHint: '浏览和克隆来自您的 GitLab 帐户的仓库。',
      loadRepos: '加载仓库',
      loadingRepos: '正在加载仓库…',
